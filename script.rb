...
def run_script(template_file, directory, output)
  ...
  # Check to see if anyone has exactly the same sheet
  students.each do |student|
    matches = students.map do |s|
      unless s.eql?(student)
        ...
        (s1_cells.length - (s1_cells - s2_cells).length) * 100 / s1_cells.length
      end
    end

    matches = matches.compact.sort.last

    student[:details][:percent] = matches
